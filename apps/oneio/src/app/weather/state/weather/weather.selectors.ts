...
export const getWeatherState = createFeatureSelector<
  WeatherPartialState,
  State
>(WEATHER_FEATURE_KEY);
...
export const getAllWeather = createSelector(getWeatherState, (state: State) =>
  selectAll(state)
);
...
export const getWeatherEntitiesArray = createSelector(
  getWeatherState,
  (state: State) => Object.entries(selectEntities(state))
);
